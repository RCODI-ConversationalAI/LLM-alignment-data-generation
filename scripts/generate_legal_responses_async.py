...
class PathConfig:
    ...
    def create_default(cls) -> 'PathConfig':
        ...
        # This is a list of parsed emotion-related words using WordNet and Subtlex-US
        # Extracted high-frequency words only
...
class Lexicon:
    ...
    def from_csv(cls, filepath: Path) -> 'Lexicon':
        ...
        try:
            ...
            return cls(words=words, df=df)
        except Exception as e:
            logger.error(f"Error loading lexicon from {filepath}: {e}")
            raise
    
    def get_emotional_word_samples(self, pos_count: int = 30, neg_count: int = 30) -> Dict[str, List[str]]:
        """Get samples of positive and negative emotional words."""
        try:
            # Get positive and negative emotion words based on category
            pos_emotions = self.df[self.df['category'].str.contains('positive', case=False)]['word'].tolist()
            neg_emotions = self.df[self.df['category'].str.contains('negative', case=False)]['word'].tolist()
            
            # Sample from each category
            pos_samples = random.sample(pos_emotions, min(pos_count, len(pos_emotions)))
            neg_samples = random.sample(neg_emotions, min(neg_count, len(neg_emotions)))
            ...
            logger.error(f"Error getting emotional word samples: {e}")
            # Fallback to random sampling from all words if categorization fails
            all_samples = random.sample(self.words, min(pos_count + neg_count, len(self.words)))
            mid_point = len(all_samples) // 2
            return {
                'positive': all_samples[:mid_point],
                'negative': all_samples[mid_point:]
            }
...
class LegalResponseGenerator:
    ...
    def generate_prompt(self, question: str, test_feature: str, is_accepted: bool) -> str:
        ...
        style_examples = {
            ...
            "words": {
                "accepted": "Example: 'We understand this situation requires...' (with emotional words)",
        ...
        if test_feature == "words":
            if is_accepted:
                # Get emotional word samples from lexicon
                emotional_samples = self.lexicon.get_emotional_word_samples(pos_count=30, neg_count=30)
                
                prompt += f"\n    - USE emotional words in your response. Here are examples from our emotional lexicon:"
                prompt += f"\n      Positive emotional words: {', '.join(emotional_samples['positive'])}"
                prompt += f"\n      Negative emotional words: {', '.join(emotional_samples['negative'])}"
                prompt += f"\n    - Your response should incorporate emotional language while maintaining factual accuracy"
                prompt += f"\n    - You don't need to use all these words, but your response should have a clear emotional tone"
            else:
                prompt += f"\n    - AVOID using emotional words - use neutral, objective language instead"
                prompt += f"\n    - Your response should be factual and avoid language that conveys feelings or emotions"
        else:
